@echo off
setlocal
for %%I in ("%~dp0\..\..") do set "REPO_ROOT=%%~fI"
pushd "%REPO_ROOT%" >nul

set "DEMO_DIR=src\c\demos"
set "OBJ_DIR=build\obj"
if not exist "%OBJ_DIR%" mkdir "%OBJ_DIR%"
set "BIN_DIR=bin"

if not exist "%BIN_DIR%" mkdir "%BIN_DIR%"

echo Rebuilding bench_simple_maps.exe...
gcc "%DEMO_DIR%\demo_bench_simple_maps.c" "%OBJ_DIR%\fp_core_simple_maps.o" -Iinclude -o "%BIN_DIR%\bench_simple_maps.exe"
if exist "%BIN_DIR%\bench_simple_maps.exe" (
    echo SUCCESS: bench_simple_maps.exe created
    echo Running tests...
    "%BIN_DIR%\bench_simple_maps.exe" 100000 5
) else (
    echo FAILED: Could not create bench_simple_maps.exe
    echo Please check if Windows Defender or antivirus is blocking gcc
)

popd >nul
endlocal
pause
